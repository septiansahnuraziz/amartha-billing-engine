...
type (
	...
	CacheManager interface {
		...
		Purge(string) error
		BatchPurge(matchStrings []string) error
...
// BatchPurge is used to remove all cache items that match any of the given patterns.
func (cache *cacheManager) BatchPurge(matchStrings []string) error {
	if cache.disableCaching {
		return nil
	}
	...
	for {
		...
		for _, matchString := range matchStrings {
			...
			stop = res[0].([]uint8)
			if foundKeys, ok := res[1].([]any); ok {
				allFoundKeys = append(allFoundKeys, foundKeys...)
			}

			cursor = res[0]
		}

		if len(allFoundKeys) > 0 {
			err := client.Send("DEL", allFoundKeys...)
			if err != nil {
				return err
			}
